...
class BinaryClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        #  Output should be a (N,) integer tensor.
        # ====== YOUR CODE: ======
        mark = y_proba[:,self.positive_class] >= self.threshold
        mark = mark.type(torch.int)
        # print(mark.shape, 'classify', mark[1600:1605])
        return mark
        # ========================
...
def select_roc_thresh(
    ...
    fpr, tpr, thresh = roc_curve(y.detach().numpy(), y_score.detach().numpy())
    dis = (fpr-0)**2 + (tpr-1)**2
