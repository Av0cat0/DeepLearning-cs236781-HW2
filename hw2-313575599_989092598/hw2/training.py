    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            # ====== YOUR CODE: ======
            # train_r = self.train_epoch(dl_train, verbose=False)
            train_r = self.train_epoch(dl_train, **kw)
            train_loss.append(torch.mean((torch.tensor(train_r.losses))).item()) # float
            # train_loss.append(train_r.losses)
            train_acc.append(train_r.accuracy.item())
            
            test_r = self.test_epoch(dl_test, **kw)
            # test_r = self.test_epoch(dl_test, verbose=False)
            # test_loss.append(test_r.losses)
            test_loss.append(torch.mean((torch.tensor(test_r.losses))).item())
            test_acc.append(test_r.accuracy.item())
            ...
            if best_acc is None or test_r.accuracy > best_acc:
                # ====== YOUR CODE: ======
